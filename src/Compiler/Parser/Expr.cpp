...
Errors parse_expr_16(ParseHelper &ph, StmtBase *&loc) {
  ...
  while (ph.accept(TOK_COMMA)) {
    ...
    rhs = new StmtExpr(lhs, oper, rhs, idx);
    lhs = nullptr;
  }
  if (rhs->type() == GT_EXPR) {
...
Errors parse_expr_15(ParseHelper &ph, StmtBase *&loc) {
  ...
  while (exprs.size() > 0) {
    ...
    oper = opers.back();
    opers.pop_back();
    exprs.push_back(new StmtExpr(lhs, oper, rhs, oper->pos));
  }
  loc = lhs;
  return E_OK;
fail:
...
Errors parse_expr_14(ParseHelper &ph, StmtBase *&loc) {
  ...
  while (ph.accept(TOK_ADD_ASSN, TOK_SUB_ASSN) ||
    ...
    lhs = new StmtExpr(lhs, oper, rhs, idx);
    rhs = nullptr;
  }
...
Errors parse_expr_02(ParseHelper &ph, StmtBase *&loc) {
  ...
  if (ph.accept(TOK_XINC, TOK_XDEC)) {
    ...
    oper = ph.peak();
    ph.next();
    loc = new StmtExpr(lhs, oper, nullptr, idx);
    return E_OK;
  }
...
Errors parse_term(ParseHelper &ph, StmtBase *&loc, const bool make_const) {
  if (ph.acceptd()) {
    if (make_const && ph.peakt() == TOK_IDENT)
      ph.sett(TOK_STRING);
    loc = new StmtSimple(ph.peak());
    ph.next();
  } else if (!make_const && ph.accept(TOK_FUNC)) {
    if (parse_func_decl(ph, loc) != E_OK)
      goto fail;
