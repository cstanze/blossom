...
Errors parse_impl_decl(ParseHelper &ph, StmtBase *&loc) {
  bool argsDone = false;
  StmtBase *args = nullptr, *body = nullptr, *in = nullptr, *name = nullptr;
  size_t idx = ph.peak()->pos;

  if (!ph.accept(TOK_IMPL)) {
    Err::set(E_PARSE_FAIL, ph.peak()->pos, "Expected 'impl' keyword");
  ...
  if (!ph.accept(TOK_DOT)) {
    Err::set(E_PARSE_FAIL, ph.peak()->pos, "Expected '.' before member name");
    return parseFail(args, body, in, name);
  }
  ph.next();
  ...
  while (true) {
    ...
    if (argsDone) {
      Err::set(E_PARSE_FAIL, ph.peak()->pos, "Expected ')' after arguments");
      return parseFail(args, body, in, name);
    }
  ...
  loc = new StmtImplDef((StmtFnDefArgs *)args, (StmtBlock *)body,
                        (StmtSimple *)in, (StmtSimple *)name, idx);
  return E_OK;
}