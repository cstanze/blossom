...
Errors tokenize(const std::string &src, lex::toks_t &toks,
  ...
  while (i < end_idx) {
    ...
    if (CURR(src) == '#' && i == 0) { // handle shebang
      comment_line = true;
      ++i;
      continue;
    }

    if (CURR(src) == '/' && NEXT(src) == '/') { // handle single line comment
      comment_line = true;
      i += 2;
      continue;
    }
    ...
    if (CURR(src) == '\"' || CURR(src) == '\'' || CURR(src) == '`') {
      ...
      toks.emplace_back(i - str.size(), TOK_STRING, str);
      continue;
    }
...
static std::string get_name(const std::string &src, size_t &i) {
  ...
  buf.push_back(src[i++]);
  while (i < src_len) {
    if (!isalnum(CURR(src)) && CURR(src) != '_')
      break;
    buf.push_back(src[i++]);
  }
...
static Errors get_const_str(const std::string &src, size_t &i,
  ...
  while (i < src_len) {
    ...
    if (CURR(src) == quote_type && continuous_backslash % 2 == 0)
      break;
    buf.push_back(src[i++]);
