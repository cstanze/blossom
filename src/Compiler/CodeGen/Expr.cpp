...
bool StmtExpr::codegen(Bytecode& bc) const {
  ...
  if (m_or_blk) {
    ...
    if (m_or_blk_var)
      bc.adds(m_or_blk->idx(), OP_PUSH_JMPN, ODT_STRING, m_or_blk_var->data);
  }
  ...
  if (m_oper->type == TOK_ADD)
    bc.adds(m_oper->pos, OP_LOAD, ODT_STRING, TokStrs[m_oper->type]);
  ...
  if (m_oper->type == TOK_LAND || m_oper->type == TOK_LOR) {
    ...
    for (size_t i = 0; i < jmps_to_rem; ++i)
      jmp_locs.pop_back();
    goto done;
  }
  ...
  if (m_oper->type == TOK_ASSN) {
    ...
    bc.adds(m_oper->pos, m_oper->type == TOK_OPER_FN ? OP_FNCL : OP_MEM_FNCL,
            ODT_STRING, m_rhs ? fn_call_args.back() : "0");
    if (m_rhs)
      fn_call_args.pop_back();
    goto done;
  }

  // skip extras (functions and dummy)
  if (m_oper->type < TOK_OPER_FN || m_oper->type >= TOK_OPER_SUBS) {
    bc.adds(m_oper->pos, OP_MEM_FNCL, ODT_STRING, m_rhs ? "00" : "0");
  }
done:
  if (m_or_blk) {
