 ...
 *	  tokens from source code string store them as a vector of tok_t
 *(toks_t); sets err::set() for setting errors
 *
...
Errors tokenize(const std::string &src, lex::toks_t &toks,
                ...
                const size_t &begin_idx = 0, size_t end_idx = -1);
...
inline bool tok_type_is_data(const int type) {
  return type == TOK_INT || type == TOK_FLOAT || type == TOK_STRING ||
         type == TOK_IDENT || type == TOK_TRUE || type == TOK_FALSE ||
 ...
 *
 * \param type - From enum TokType
 * \return true If the type is one of possible assignment operators, false if it
 * isn't
 */
inline bool tok_type_is_assign(const int type) {
  return (type == TOK_ASSN || type == TOK_ADD_ASSN || type == TOK_SUB_ASSN ||
          type == TOK_MUL_ASSN || type == TOK_DIV_ASSN ||
          type == TOK_MOD_ASSN || type == TOK_BAND_ASSN ||
          type == TOK_BOR_ASSN || type == TOK_BNOT_ASSN ||
          type == TOK_BXOR_ASSN || type == TOK_LSHIFT_ASSN ||
 ...
 *
 * \param tok Pointer to a tok_t
 * \return true If the type is one of assignment operator tokens, false if it
 * isn't
 */
