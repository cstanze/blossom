...
class var_int_iterable_t : public VarBase {
...
VarBase *range(VMState &vm, const FnData &fd) {
  ...
  if (fd.args.size() > 2)
    mpz_set(begin, INT(lhs_base)->get());
  else
    mpz_set_si(begin, 0);
  if (rhs_base)
    mpz_set(end, INT(rhs_base)->get());
  else
...
INIT_MODULE(utils) {
  ...
  vm.register_type<var_int_iterable_t>("IntIterable", src_id, idx);

  vm.add_native_typefn<var_int_iterable_t>("next", int_iterable_next, 0, src_id,
                                           idx);
